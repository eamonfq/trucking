...
export const Input = forwardRef<HTMLInputElement, InputProps>(function Input(
  ...
  return (
    <label className="grid gap-2 text-xs font-medium text-ink-700" htmlFor={inputId}>
      {label}
      <input
        ref={ref}
        id={inputId}
        ...
        {...props}
      />
      {(error || hint) && <span id={descriptionId} className={cn("text-xs font-normal", error ? "text-danger" : "text-ink-500")}>{error ?? hint}</span>}
    </label>
  );
});
