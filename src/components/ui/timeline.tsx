  ...
  return (
    <ol className="grid" aria-label="Historial del envío">
      {events.map((event, index) => {
        ...
        return (
          <li key={`${event.status}-${event.occurredAt}`} className="relative grid grid-cols-[2rem_1fr] gap-3 pb-7 last:pb-0">
            ...
            <div className="pt-1"><div className="flex flex-wrap items-baseline justify-between gap-2"><p className="text-sm font-bold text-navy-900">{getStatusLabel(event.status)}</p><time className="text-xs text-navy-500">{formatDate(event.occurredAt)}</time></div><p className="mt-1 text-sm text-navy-500">{event.location} · {event.description}</p></div>
          </li>
        );
      })}
    </ol>
