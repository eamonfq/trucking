...
export default async function AttentionPage() {
  ...
  for(const invoice of invoices) if(invoice.status==="pendiente-pago-destino"||invoice.status==="pago-reportado"||isInvoiceOverdue(invoice))items.push({id:invoice.id,kind:"Pagos",title:invoice.number,detail:`${invoice.status==="pendiente-pago-destino"?"Pendiente de pago en destino":invoice.status==="pago-reportado"?"Reporte pendiente de validación":"Factura vencida"} · ${formatUsd(invoiceTotal(invoice))}`,customer:customer(invoice.userId),at:invoice.paymentReport?.reportedAt??invoice.dueAt,href:`/admin/facturas?invoice=${encodeURIComponent(invoice.id)}`,priority:true});
