...
export const externalPaymentMethodSchema = z.string().trim().transform(value=>value.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'')).pipe(z.enum(['efectivo','tarjeta','transferencia','deposito']));
export const paymentReportSchema = z.object({ amount: z.coerce.number().positive(), method: externalPaymentMethodSchema, reference: z.string().trim().max(160).default(""), receiptName: z.string().optional() });
