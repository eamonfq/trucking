...
export function CustomerDetail({ initialUser, initialAddresses, initialRecipients, boxes, shipments, invoices }: { initialUser: User; initialAddresses: Address[]; initialRecipients: Recipient[]; boxes: Box[]; shipments: Shipment[]; invoices: Invoice[] }) {
  ...
  return <div className="grid gap-6">
    ...
    <Dialog open={confirmation === "password"} onClose={() => { setConfirmation(null); setTemporaryPassword(""); }} title="Restablecer contraseña" description="Se generará una contraseña temporal y se enviará al correo del cliente.">{temporaryPassword ? <div><div className="rounded-2xl bg-cream-100 p-5 text-center"><p className="text-xs font-bold uppercase tracking-wider text-navy-500">Contraseña temporal</p><p className="mt-2 font-mono text-xl font-bold text-navy-950">{temporaryPassword}</p></div><p className="mt-3 text-xs leading-5 text-navy-500">Copia este valor ahora. Por seguridad dejará de mostrarse al cerrar.</p><div className="mt-5 flex justify-end"><Button onClick={() => { navigator.clipboard.writeText(temporaryPassword); showToast({ title: "Contraseña copiada" }); }}>Copiar contraseña</Button></div></div> : <div className="flex justify-end gap-2"><Button variant="ghost" onClick={() => setConfirmation(null)}>Cancelar</Button><Button loading={busy} onClick={confirmAccountAction}>Generar y enviar</Button></div>}</Dialog>
...
function BoxesPanel({ boxes }: { boxes: Box[] }) { return <DataTable rows={boxes} getRowKey={(row) => row.id} emptyTitle="Sin cajas" emptyDescription="Las cajas recibidas o prealertadas aparecerán aquí." columns={[{ key: "code", header: "Caja", render: (row) => <Link href={`/admin/cajas/${row.id}`} className="font-bold text-orange-600">{row.code}</Link> }, { key: "category", header: "Categoría", render: (row) => BOX_CATEGORIES.find((item) => item.id === row.categoryId)?.name }, { key: "weight", header: "Peso", render: (row) => `${row.weightLb} lb` }, { key: "status", header: "Estado", render: (row) => <StatusBadge status={row.status} /> }, { key: "received", header: "Recepción", render: (row) => row.receivedAt ? formatDate(row.receivedAt) : getStatusLabel(row.status) }]} />; }
