...
export function ReceptionForm({ users, excessPolicy, rates, defaultCustomerId }: { users: User[]; excessPolicy: string; rates: BoxCategory[]; defaultCustomerId?: string }) {
  ...
  const { register, handleSubmit, control, reset, formState: { errors, isSubmitting } } = useForm<ReceptionInput>({ resolver: zodResolver(receptionSchema), defaultValues: { customer: defaultCustomerId ?? "", reject: false, overrideCategory: "", overrideReason: "", rejectionReason: "" } });
  ...
  return <form onSubmit={handleSubmit(async (data) => {
    ...
    <div className="grid content-start gap-5 rounded-card border border-stone-200 bg-white p-6">
      <Select label="Cliente o casillero" error={errors.customer?.message} options={[{ value: "", label: "Selecciona" }, ...users.filter((user) => user.role === "cliente" && user.active).map((user) => ({ value: user.id, label: `${user.lockerCode} · ${user.firstName} ${user.paternalLastName}` }))]} {...register("customer")} />
      ...
      <Input label="Foto de recepción" type="file" accept="image/*" onChange={(event) => setPhotoName(event.target.files?.[0]?.name ?? "")} />
