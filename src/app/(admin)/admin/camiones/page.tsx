...
export default async function TrucksPage() { await requireAdminUser(["camiones"]); const [trucks, drivers] = await Promise.all([logisticsService.getTrucks(), logisticsService.getDrivers()]); return <><SectionTitle eyebrow="Guías máster" title="Camiones" description="Planifica salidas, controla capacidad y coordina cada etapa del recorrido a México." /><div className="mt-7"><TruckManager initial={trucks} initialDrivers={drivers} /></div></>; }
