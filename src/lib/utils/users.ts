...
export function toClientUser(user: User): ClientUser {
  ...
  return { ...(role === "admin" && user.adminPermissions !== undefined ? {adminPermissions:[...user.adminPermissions]} : {}), ...(role === "operador" ? {warehouseGrants:structuredClone(user.warehouseGrants??[])} : {}), id, role, firstName, paternalLastName, maternalLastName, email, phone, lockerCode, rfc, active, activity: structuredClone(activity) };
}
