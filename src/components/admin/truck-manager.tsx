...
import { DEFAULT_TRUCK_CAPACITY, DESTINATION_CITIES, OPERATION_ORIGIN } from "@/lib/config/operations";
...
export function TruckManager({ initial, initialDrivers }: { initial: Truck[]; initialDrivers: Driver[] }) {
  ...
  const { register, handleSubmit, reset, control, formState: { errors, isSubmitting } } = useForm<TruckInput>({
    ...
    defaultValues: { driverId: "", destinationCity: DESTINATION_CITIES[0], capacity: { ...DEFAULT_TRUCK_CAPACITY }, notes: "" },
  });
  ...
  return <div className="grid gap-6">
    <form onSubmit={handleSubmit(async (data) => {
      ...
      reset({ driverId: "", destinationCity: DESTINATION_CITIES[0], capacity: { ...DEFAULT_TRUCK_CAPACITY }, notes: "" });
      showToast({ title: "Camión creado", description: `${result.truck.code} quedó planificado hacia ${result.truck.destinationCity}.` });
