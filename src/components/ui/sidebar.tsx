...
export function Sidebar({ items, footer }: { items: SidebarItem[]; footer?: React.ReactNode }) {
  return (
    <aside className="flex min-h-full w-full flex-col rounded-card bg-navy-950 p-4 text-white shadow-lift lg:w-72">
      <div className="flex items-center gap-3 border-b border-white/10 px-2 pb-5"><span className="grid size-10 place-items-center rounded-xl bg-orange-500 font-display text-sm font-extrabold">A&L</span><div><p className="font-display text-sm font-bold">Trucking Logistics</p><p className="text-xs text-white/55">Panel operativo</p></div></div>
