...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const [boxes, recipients, shipments, trucks, users, locations] = await withStore(()=>Promise.all([logisticsService.getBoxes(), logisticsService.getRecipients(), logisticsService.getShipments(), logisticsService.getTrucks(), logisticsService.getUsers(),getWarehouseAdministration()]));
