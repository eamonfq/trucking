...
export default async function Page({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{grupo?:string}>}){
 ...
 return <>{group.map((b,i)=><PackageLabel key={b.id} controls={i===0} code={b.code} receptionCode={b.receptionGroup?.code} dimensions={b.dimensions} weightLb={b.weightLb} lockerCode={customer?.lockerCode??""} recipient={b.recipientSnapshot} position={b.receptionGroup?`${b.receptionGroup.index}/${b.receptionGroup.total}`:"1/1"}/>)}</>;
}
