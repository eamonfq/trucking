...
export async function GET(_request: Request, {params}: {params:Promise<{id:string}>}) {
  ...
  const [rows] = await pool().execute<RowDataPacket[]>("SELECT original_name,mime_type,content,sha256 FROM private_files WHERE id=? AND (?='admin' OR owner_id=?)",[id,session.role,session.userId]);
  const file = rows[0];
