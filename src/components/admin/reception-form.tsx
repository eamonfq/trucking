...
export function ReceptionForm({ users, prealerts = [], excessPolicy, excessFeeUsd = 0, rates, defaultCustomerId, defaultPrealertId, weightPricing = DEFAULT_WEIGHT_PRICING, locations=[], origins=[] }: { origins?:{id:string;name:string}[]; locations?:{id:string;name:string}[]; weightPricing?: WeightPricing; users: User[]; prealerts?: Box[]; excessPolicy: string; excessFeeUsd?: number; rates: BoxCategory[]; defaultCustomerId?: string; defaultPrealertId?: string }) {
  ...
  const resolvedPieces=resolveReceptionPieces({...dims,weightLb:Number(values.weightLb)||0,customPriceUsd:customPrice},pieces.slice(0,quantity-1),shared);
  const piecePrices=resolvedPieces.map(p=>{try{const category=suggestCategory(p,p.weightLb,rates).category;return calculateBilling(mode,p,p.weightLb,weightPricing,mode==="manual"?p.customPriceUsd:category?.priceUsd??0).amountUsd;}catch{return 0;}});
  const total=(billing?.amountUsd??0)+surcharge+piecePrices.reduce((a,b)=>a+b,0);
  ...
  return <form onSubmit={handleSubmit(async (data) => {
    ...
    <div className="grid min-w-0 gap-4">
      ...
      <section className="grid gap-3 rounded-2xl border border-stone-200 bg-white p-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl bg-stone-50 px-4 py-3" aria-live="polite"><div><p className="text-[10px] font-semibold uppercase tracking-wider text-navy-400">{quantity>1?"Cálculo · paquete 1":"Cálculo por pieza"}</p><h3 className="mt-1 text-sm font-semibold text-navy-950">{mode==="peso"?"Cobro por libra":mode==="manual"?"Cotización especial":rates.find(rate=>rate.id===values.overrideCategory)?.name??suggestion?.category?.name??"Precio fijo"}</h3><p className="mt-1 text-xs leading-5 text-navy-500">{mode==="peso"?`Real: ${Number(values.weightLb)||0} lb · Dimensional: ${billing?.dimensionalWeightLb.toFixed(2)??"—"} lb`:mode==="manual"?"Registra el precio acordado por pieza.":reasonLabel?`Categoría ajustada por ${reasonLabel}.`:"Completa las medidas y el peso."}</p>{surcharge>0&&<p className="text-xs text-navy-500">Recargo de primera pieza: {formatUsd(surcharge)}</p>}</div><div className="text-right"><p className="text-lg font-semibold tabular-nums text-navy-950">{billing?formatUsd(billing.amountUsd):"—"}</p><p className="mt-1 text-[11px] text-navy-500">{mode==="peso"&&billing?`${billing.billableWeightLb} lb × ${formatUsd(weightPricing.pricePerLbUsd)}`:"Por pieza"}</p></div></div>
        ...
        {quantity>1&&<PackageMeasurements shared={shared} page={piecePage} onPageChange={setPiecePage} pieces={resolvedPieces} prices={piecePrices} manual={mode==="manual"} onChange={(i,field,value)=>setPieces(items=>items.map((item,j)=>j===i?{...item,[field]:value}:item))}/>}
