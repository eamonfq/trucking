...
import { Edit3, FileDown, PackagePlus, Trash2 } from "lucide-react";
import { z } from "zod";
import { assignBoxToTruck, removeBoxFromTruck, transitionTruckState, updateTruck } from "@/lib/auth/admin-actions";
import { BOX_CATEGORIES } from "@/lib/config/box-categories";
import { DESTINATION_CITIES, OPERATION_ORIGIN } from "@/lib/config/operations";
...
export function TruckDetailManager({ initialTruck, initialAssigned, initialAvailable, users, drivers, shipments }: { initialTruck: Truck; initialAssigned: Box[]; initialAvailable: Box[]; users: User[]; drivers: Driver[]; shipments: Shipment[] }) {
  const [truck, setTruck] = useState(initialTruck);
  const [assigned, setAssigned] = useState(initialAssigned);
  const [available, setAvailable] = useState(initialAvailable);
  const [selectedBoxId, setSelectedBoxId] = useState(initialAvailable[0]?.id ?? "");
  ...
  const { register, handleSubmit, reset, formState: { errors, isSubmitting } } = useForm<TruckInput>({
    ...
    defaultValues: { plate: truck.plate, driverId: truck.driverId, departureDate: truck.departureDate, destinationCity: truck.destinationCity as TruckInput["destinationCity"], capacity: truck.capacity, notes: truck.notes },
  });
  const action = TRUCK_ACTIONS[truck.status];
  const usage = useMemo(() => Object.fromEntries(BOX_CATEGORIES.map((category) => [category.id, assigned.filter((box) => box.categoryId === category.id).length])), [assigned]);
  const shipmentByBox = (boxId: string) => shipments.find((shipment) => shipment.boxIds.includes(boxId));

  const addBox = async () => {
    if (!selectedBoxId) return;
    setBusy(true);
    const result = await assignBoxToTruck(truck.id, selectedBoxId);
    setBusy(false);
    if (!result.ok) return showToast({ title: "No se pudo asignar la caja", description: result.error, variant: "error" });
    setTruck({ ...result.truck });
    setAssigned((current) => [...current, { ...result.box }]);
    setAvailable((current) => current.filter((box) => box.id !== result.box.id));
    showToast({ title: "Caja asignada", description: `${result.box.code} quedó vinculada a ${result.truck.code}.` });
  };
  ...
  const removeBox = async (boxId: string) => {
    ...
    setAvailable((current) => [...current, { ...result.box }]);
  ...
  return <div className="grid gap-6">
    <div className="flex flex-wrap items-center justify-between gap-3 rounded-card border border-stone-200 bg-white p-5"><div className="flex flex-wrap items-center gap-3"><StatusBadge status={truck.status} /><span className="text-sm text-navy-500">{truck.plate} · {truck.driverName} · salida {truck.departureDate}</span></div><div className="flex flex-wrap gap-2">{truck.status === "planificado" && <Button variant="secondary" onClick={() => { reset({ plate: truck.plate, driverId: truck.driverId, departureDate: truck.departureDate, destinationCity: truck.destinationCity as TruckInput["destinationCity"], capacity: truck.capacity, notes: truck.notes }); setEditOpen(true); }}><Edit3 className="size-4" />Editar viaje</Button>}<a href={`/api/camiones/${truck.id}/manifiesto`} className="inline-flex min-h-11 items-center gap-2 rounded-full border border-stone-200 px-4 text-sm font-bold"><FileDown className="size-4" />Manifiesto PDF</a>{action && <Button onClick={() => setTransitionOpen(true)}>{action.label}</Button>}</div></div>

    <section><h2 className="font-display text-xl font-bold">Capacidad por categoría</h2><div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-5">{BOX_CATEGORIES.map((category) => { const used = usage[category.id] ?? 0; const total = truck.capacity[category.id]; return <div key={category.id} className="rounded-2xl border border-stone-200 bg-white p-4"><p className="text-xs font-bold uppercase tracking-wider text-navy-500">{category.name}</p><p className="mt-2 font-display text-2xl font-bold">{used} <span className="text-sm text-navy-400">/ {total}</span></p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-stone-100"><div className="h-full rounded-full bg-orange-500" style={{ width: `${Math.min(100, total ? used / total * 100 : 100)}%` }} /></div></div>; })}</div></section>

    {(["planificado", "cargando"] as string[]).includes(truck.status) && <section className="rounded-card bg-navy-950 p-5 text-white"><div className="flex items-center gap-3"><PackagePlus className="size-5 text-orange-400" /><div><h2 className="font-display text-lg font-bold">Agregar caja disponible</h2><p className="mt-1 text-sm text-white/60">La capacidad se valida antes de asignar.</p></div></div><div className="mt-4 flex flex-col gap-3 sm:flex-row"><select aria-label="Caja disponible" value={selectedBoxId} onChange={(event) => setSelectedBoxId(event.target.value)} className="min-h-11 flex-1 rounded-xl bg-white px-3 text-sm text-navy-950"><option value="">Selecciona una caja</option>{available.map((box) => <option key={box.id} value={box.id}>{box.code} · {BOX_CATEGORIES.find((category) => category.id === box.categoryId)?.name}</option>)}</select><Button disabled={!selectedBoxId} loading={busy} onClick={addBox}>Agregar caja</Button></div></section>}

    <section><div className="flex items-end justify-between gap-4"><div><h2 className="font-display text-xl font-bold">Cajas asignadas</h2><p className="mt-1 text-sm text-navy-500">{assigned.length} cajas vinculadas a esta guía máster.</p></div></div>{assigned.length ? <div className="mt-4 overflow-x-auto rounded-card border border-stone-200 bg-white"><table className="w-full min-w-[760px] text-left text-sm"><thead className="bg-cream-100 text-xs uppercase tracking-wider text-navy-500"><tr><th className="p-4">Caja</th><th className="p-4">Cliente</th><th className="p-4">Categoría</th><th className="p-4">Peso</th><th className="p-4">Envío</th><th className="p-4 text-right">Acción</th></tr></thead><tbody className="divide-y divide-stone-200">{assigned.map((box) => { const customer = users.find((user) => user.id === box.userId); return <tr key={box.id}><td className="p-4 font-bold">{box.code}</td><td className="p-4">{customer ? `${customer.firstName} ${customer.paternalLastName}` : "Cliente"}</td><td className="p-4">{BOX_CATEGORIES.find((category) => category.id === box.categoryId)?.name}</td><td className="p-4">{box.weightLb} lb</td><td className="p-4">{shipmentByBox(box.id)?.code ?? "Sin envío"}</td><td className="p-4 text-right">{(["planificado", "cargando"] as string[]).includes(truck.status) && <button onClick={() => removeBox(box.id)} className="inline-flex min-h-10 items-center gap-2 rounded-full px-3 font-bold text-danger-700 hover:bg-danger-50"><Trash2 className="size-4" />Quitar</button>}</td></tr>; })}</tbody></table></div> : <div className="mt-4"><EmptyState title="Todavía no hay cajas asignadas" description="Selecciona una caja disponible para preparar la carga de este camión." /></div>}</section>
