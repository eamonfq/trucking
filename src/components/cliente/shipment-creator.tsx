...
export function ShipmentCreator({ boxes, recipients, excessPolicy, rates }: { boxes: Box[]; recipients: Recipient[]; excessPolicy: string; rates: BoxCategory[] }) {
  const category = (id: Box["categoryId"]) => rates.find((item) => item.id === id); const [done, setDone] = useState(false); const { register, handleSubmit, control, formState: { errors, isSubmitting } } = useForm<Input>({ resolver: zodResolver(createShipmentSchema), defaultValues: { boxIds: [], deliveryMethod: "sucursal" } }); const selected = useWatch({ control, name: "boxIds" }) ?? []; const selectedBoxes = boxes.filter((box) => selected.includes(box.id)); const total = selectedBoxes.reduce((sum, box) => sum + (category(box.categoryId)?.priceUsd ?? 0), 0); const blocked = selectedBoxes.some((box) => box.status === "excede-categoria");
