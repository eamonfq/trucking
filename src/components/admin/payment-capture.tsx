...
export type PaymentChoice = { method: "clover" | "efectivo" | "destino" | "tarjeta" | "transferencia" | "deposito"; warehouseId?:string; amount: string; reference: string };
export function PaymentCapture({value,onChange,total,allowDestination=true,locations=[]}:{value:PaymentChoice;onChange:(value:PaymentChoice)=>void;total:number;allowDestination?:boolean;locations?:{id:string;name:string}[]}){
 ...
 useEffect(()=>{cloverAvailability().then(result=>setCloverEnabled(result.enabled)).catch(()=>{});},[]);
 const options=[{value:"efectivo" as const,label:"Efectivo",icon:Banknote},...(allowDestination?[{value:"destino" as const,label:"En destino",icon:MapPin}]:[]),{value:"tarjeta" as const,label:"Tarjeta externa",icon:CreditCard},...(cloverEnabled?[{value:"clover" as const,label:"Clover · cobrar",icon:CreditCard}]:[]),{value:"transferencia" as const,label:"Transferencia",icon:Banknote},{value:"deposito" as const,label:"Depósito",icon:Banknote}];
 return <fieldset className="grid min-w-0 gap-3">
  ...
  <p className={`rounded-xl p-3 text-sm leading-5 ${value.method==="destino"?"bg-amber-50 text-amber-900":"bg-emerald-50 text-emerald-900"}`}>{value.method==="clover"?"Se abrirá el formulario seguro para cobrar el total. No se marcará pagado hasta que Clover lo confirme.":value.method==="destino"?"Pendiente de pago en destino. Se generará un folio de acuerdo; no registra dinero recibido.":"Al guardar confirmas que recibiste el total. Se generará un folio de pago único y quedará registrado en la factura."}</p>
 </fieldset>;
}
